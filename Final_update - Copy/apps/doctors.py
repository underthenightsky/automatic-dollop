...
def app():
    ...
    user_collection = db["Doctors"]

    # Find all documents in the collection
    users = user_collection.find({}, {"username": 1, "medical_speciality": 1})
    # else :    
    # # # Load data
    ...
    spec_list = [user["medical_speciality"] for user in users]
    Department_1 = st.sidebar.multiselect('Select Department', spec_list)
    username_list = [doctor["username"] for doctor in doctors]
    # Filter data
    filtered_df = [user for user in spec_list if user in Department_1]
    filtered_doctors = []
    for i, specialties in enumerate(Department_1):
        if any(specialty in Department_1 for specialty in specialties):
            filtered_doctors.append({
                "username": username_list[i]
            })
    # Display results=
    for user in filtered_doctors():
        #st.write(f"## {row['name']} ({row['medical_speciality']})")
        #st.write(f"Hospital Name: {row['Hospital_name']}")
        name = user["username"]
